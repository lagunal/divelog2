      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              context.go('/dives/new', extra: session);
            },
          ),
          IconButton(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSection('Detalles de la Sesión', [
              ...
              _buildDetailRow(
                  'Int. Superficie',
                  session.surfaceInterval != null
                      ? '${session.surfaceInterval} min'
                      : null),
              _buildDetailRow(
                  'Tiempo Total',
                  session.totalDiveTime != null
                      ? '${session.totalDiveTime} min'
                      : null),
            ]),
            const SizedBox(height: 16),
            ...
            _buildSection('Tiempos Acumulados', [
              _buildDetailRow(
                  'Supervisión',
                  session.accumulatedSupervisionTime != null
                      ? '${session.accumulatedSupervisionTime} min'
                      : null),
              _buildDetailRow(
                  'Buceo',
                  session.accumulatedDiveTime != null
                      ? '${session.accumulatedDiveTime} min'
                      : null),
            ]),
